---
title: AnyLabeling - Smart Image Labeling Tool
---

import Link from 'next/link'
import cn from 'clsx'
import { useState } from 'react'
import Image from 'next/image'
import { motion } from "framer-motion"

import { Features, Feature } from '@components/features'

import styles from './index.module.css'

export function File({ name, className, ...props }) {
  return <span className={cn(styles.file, className)} {...props}>
    {name}
  </span>
}

export function Comparison({ a, b, invert, ...props }) {
  return <div className={cn(styles.comparison, invert && styles.invert)} {...props}>
    <div>{a}</div>
    <svg xmlns="http://www.w3.org/2000/svg" width="1.2em" viewBox="0 0 20 20" fill="currentColor">
      <path fillRule="evenodd" d="M12.293 5.293a1 1 0 011.414 0l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-2.293-2.293a1 1 0 010-1.414z" clipRule="evenodd" />
    </svg>
    <div>{b}</div>
  </div>
}

export function I18n() {
  const [active, setActive] = useState('')
  return <Comparison a={
    <div style={{ display: 'flex', flexDirection: 'column', gap: '.5rem' }}>
      <File name="/hello.en-US.mdx" className={active === 'en' && styles.active} onPointerOver={() => setActive('en')}/>
    </div>
  } b={
    <div
      style={{ display: 'flex', flexDirection: 'column' }}
      className="menu bg-white dark:bg-neutral-800 dark:ring-white dark:ring-opacity-20 shadow-lg rounded-md py-1 ring-1 ring-black ring-opacity-5 overflow-auto text-sm">
      <span className={cn("cursor-default select-none relative py-1.5 px-4 whitespace-nowrap", active === 'en' ? 'nx-text-primary-600 nx-bg-primary-50 dark:nx-bg-primary-500/10' : 'text-gray-800 dark:text-gray-100 ')} onPointerOver={() => setActive('en')}>English</span>
    </div>
  }/>
}

import docsCard from '../public/screenshot.png'
import docsCardDark from '../public/screenshot.png'

<div className="home-content">
  <div className="content-container">
    <h1 className="headline">Effortless data labeling</h1>
    <p className="subtitle">With AI support from Segment Anything and YOLO models.</p>
    <div className="anylearning-callout">
      <a className="anylearning-banner" href="https://github.com/nrl-ai/anylearning-oss" target="_blank" rel="noopener noreferrer" aria-label="Explore AnyLearning OSS on GitHub">
        <img src="/anylearning-oss-banner.webp" alt="AnyLearning open-source offline AI platform" />
      </a>
    </div>
  </div>

  <style jsx>{`
    .content-container {
      max-width: 90rem;
      padding-left: max(env(safe-area-inset-left),1.5rem);
      padding-right: max(env(safe-area-inset-right),1.5rem);
      margin: 0 auto;
    }
    .features-container {
      margin: 8rem 0 0;
      padding: 4rem 0;
      background-color: #f4f4f5;
      border-block: 1px solid #e4e4e7;
    }
    .features-container .content-container {
      margin-top: -8rem;
    }
    :global(.dark) .features-container {
      background-color: #18181b;
      border-color: #3f3f46;
    }
    .headline {
      display: inline-block;
      font-size: 3.125rem;
      font-size: min(4.375rem, max(8vw, 2.5rem));
      font-weight: 700;
      font-feature-settings: initial;
      letter-spacing: -.12rem;
      margin-left: -0.2rem;
      margin-top: 3.4rem;
      line-height: 1.1;
      background-image: linear-gradient(146deg,#000,#757a7d);
      -webkit-background-clip: text;
      -webkit-text-fill-color: transparent;
      background-clip: text;
    }
    :global(.dark) .headline {
      background-image: linear-gradient(146deg,#fff,#757a7d);
    }
    .subtitle {
      font-size: 1.3rem;
      font-size: min(1.3rem, max(3.5vw, 1.2rem));
      font-feature-settings: initial;
      line-height: 1.6;
    }
    .anylearning-callout {
      width: min(68rem, 100%);
      margin-top: 2rem;
    }
    .anylearning-banner {
      display: block;
      overflow: hidden;
      border: 1px solid #e4e4e7;
      border-radius: 0.875rem;
      background: #f4f4f5;
      box-shadow: 0 1px 2px rgb(0 0 0 / 5%), 0 18px 40px -32px rgb(9 9 11 / 45%);
      text-decoration: none;
      transition: transform 180ms ease, border-color 180ms ease, box-shadow 180ms ease;
    }
    .anylearning-banner img {
      display: block;
      width: 100%;
      height: auto;
    }
    .anylearning-banner:hover {
      transform: translateY(-2px);
      border-color: rgb(8 145 178 / 55%);
      box-shadow: 0 2px 4px rgb(0 0 0 / 6%), 0 22px 48px -32px rgb(8 145 178 / 38%);
    }
    .anylearning-banner:focus-visible {
      outline: 2px solid rgb(8 145 178);
      outline-offset: 3px;
    }
    :global(.dark) .anylearning-banner {
      border-color: #3f3f46;
      background: #18181b;
      box-shadow: 0 1px 2px rgb(0 0 0 / 30%), 0 18px 40px -30px rgb(0 0 0 / 80%);
    }
    :global(.dark) .anylearning-banner:hover {
      border-color: rgb(34 211 238 / 50%);
    }
    .nextjs-link {
      color: currentColor;
      text-decoration: none;
      font-weight: 600;
    }
    :global(#docs-card) {
      aspect-ratio: 5/3;
      color: #fff;
      text-shadow: 0 0 1rem rgba(0,0,0,0.1);
    }
    :global(#docs-card img) {
      object-fit: cover;
      object-position: left;
      position: absolute;
      left: 0;
      right: 0;
      top: 0;
      bottom: 0;
      height: 100%;
      z-index: 0;
      user-select: none;
      pointer-events: none;
    }
    :global(#docs-card img:nth-child(2)) {
      display: none;
    }
    :global(.dark #docs-card img:nth-child(2)) {
      display: initial;
    }
    :global(.dark #docs-card img:nth-child(1)) {
      display: none;
    }
    :global(#highlighting-card) {
      min-height: 500px;
      background-image: linear-gradient(to top, transparent, #fff 50%), url(/assets/syntax-highlighting.svg);
      background-size: 634px;
      background-position: -6px calc(100% + 4px);
      background-repeat: no-repeat;
    }
    :global(.dark #highlighting-card) {
      background-image: linear-gradient(to top, transparent, #202020 50%), url(/assets/syntax-highlighting.svg);
    }
    :global(.feat-darkmode) {
      min-height: 500px;
    }
    :global(.feat-darkmode h3) {
      font-size: 48px;
    }
    :global(#search-card) {
      display: flex;
      flex-direction: column;
      justify-content: center;
    }
    :global(#search-card p) {
      max-width: 320px;
    }
    :global(#search-card video) {
      position: absolute;
      right: 0;
      top: 24px;
      height: 430px;
      pointer-events: none;
      max-width: 60%;
    }
    :global(#fs-card) {
      min-height: 240px;
    }
    :global(#fs-card h3) {
      text-align: left;
      width: min(300px, 41%);
      min-width: 155px;
    }
    :global(#a11y-card) {
      background-image: url(/assets/high-contrast.png);
      background-position: -160px 160px;
    }
    @media screen and (max-width: 1300px) {
      :global(#a11y-card) {
        background-image: linear-gradient(to bottom, white, transparent), url(/assets/high-contrast.png);
      }
    }
    @media screen and (max-width: 1200px) {
      :global(#highlighting-card) {
        aspect-ratio: auto;
      }
      :global(.feat-darkmode h3) {
        font-size: 4vw;
        font-size: min(48px, max(4vw, 30px));
      }
      :global(#search-card video) {
        aspect-ratio: 5/3;
        height: auto;
      }
      .headline {
        letter-spacing: -.08rem;
      }
    }
    @media screen and (max-width: 1024px) {
      :global(#docs-card) {
        aspect-ratio: 5/3;
      }
      :global(#search-card) {
        aspect-ratio: 8/5;
      }
      :global(#search-card h3) {
        text-align: left;
      }
      :global(#highlighting-card) {
        background-size: 136%;
      }
      :global(#a11y-card) {
        background-image: url(/assets/high-contrast.png);
        background-position: center 160px;
      }
    }
    @media screen and (max-width: 768px) {
      :global(#docs-card) {
        min-height: 348px;
        width: 100%;
        aspect-ratio: 5/3;
      }
      :global(#docs-card img) {
        object-position: -26px 0;
        width: 250%;
        max-width: initial;
      }
    }
    @media screen and (max-width: 640px) {
      .anylearning-callout {
        margin-top: 1.5rem;
      }
      .anylearning-banner {
        border-radius: 0.75rem;
      }
      :global(#search-card) {
        aspect-ratio: 5/3;
        justify-content: flex-start;
        align-items: stretch;
        min-height: 350px;
      }
      :global(#search-card h3) {
        text-align: center;
      }
      :global(#search-card p) {
        max-width: 100%;
      }
      :global(#search-card video) {
        position: relative;
        margin: .75em -1.75em 0;
        max-width: calc(100% + 3.5em);
      }
      :global(.dark #search-card video) {
        mix-blend-mode: lighten;
      }
    }
  `}</style>

  <div className="features-container">
    <div className="content-container">
      <Features>
        <Feature index={0} large centered id="docs-card" href="/docs">
          <Image src={docsCard} alt="Background" loading="eager" />
          <Image src={docsCardDark} alt="Background (Dark)" loading="eager" />
        </Feature>
        <Feature index={1} centered className="card-with-border" href="/docs">
          <div className="h-full flex flex-col justify-start">
            <h3> <span style={{ fontWeight: 300 }}>Easy to Get Started</span> <br/></h3>
            <p className='text-center mb-2 mt-4'>Install and Run with Pip</p>
            <div>
              <div className={styles.optimization}>
                <div style={{ fontSize: '.9rem' }} className="leading-2 text-left">
                  <code>{`pip install anylabeling`}</code><br></br>
                  <code>{`anylabeling # Run the app`}</code>
                </div>
              </div>
            </div>
            <div>
              <div className="mt-2 mb-4">
                Or <Link href="https://github.com/vietanhdev/anylabeling/releases">Download Binary</Link>
              </div>
              <div className="subtitle">
                <Link className={styles.cta} href="/docs">Next steps <span>→</span></Link>
              </div>
            </div>
          </div>
        </Feature>
      </Features>
    </div>
  </div>
</div>
